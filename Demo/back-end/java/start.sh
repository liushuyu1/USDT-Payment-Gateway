...
# Generate front-end API config so index.html works with zero manual edits:
# local (file:// double-click) and server (nginx static hosting) both read DEMO_API_BASE.
FRONT_END_DIR="$ROOT/../../front-end"
if [ -d "$FRONT_END_DIR" ]; then
    printf '// Auto-generated by start.sh - DO NOT EDIT\nwindow.DEMO_API_BASE = "%s";\n' "${PUBLIC_BASE_URL%/}" > "$FRONT_END_DIR/api-config.js"
    echo "Front-end API config: $FRONT_END_DIR/api-config.js -> ${PUBLIC_BASE_URL%/}"
fi
